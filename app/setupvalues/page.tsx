...
const SetupValuesPage = () => {
  ...
  const renderDNSInfo = (level) => (
    <div className='mt-4 text-right'>
      {['primary DNS', 'secondary DNS', 'Dns-Over-Https'].map((label, index) => (
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        <p className='text-xl text-gray-700'>
          تفصيل مستويات الحماية المتاحة
        </p>
      </div>

      {/* Protection Levels Cards */}
      <div className='flex  justify-center gap-[90px] w-full max-w-5xl '>
        {protectionLevels.map((level) => (
          <div
            key={level.type}
            className='border-[4px] border-[#353535] rounded-3xl overflow-hidden bg-white shadow-md px-[30px] py-[40px] w-[380px]'
          >
